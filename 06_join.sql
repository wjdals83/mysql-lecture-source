-- join
-- 두 개 이상의 테이블을 관련 있는 컬럼을 통해 결합하는 데 사용된다. 
-- 두 개 이상의 테입블은 반드시 연관있는 컬럼이 존재해야 하며,
...
-- 별칭에 띄어쓰기나 특수기호가 없다면 홀따옴표(') 와 as 는 생략 가능하다. 별칭이 동일 컬럼 구분에 도움이 된다. 
select
	menu_code as'code',
    menu_name as name,
    menu_price 'price'
...
from
	tbl_menu a
join tbl_category b 
on a.category_code = b.category_code;		-- 다시 보기!
