...
-- cte : 쿼리 내부 (1개의 쿼리문) 에서 whth ~ as 절을 사용해서 "임시" 로 정의되는 테이블

-- select 쿼리문을 저장한 객체로 가상테이블 이라고 불린다. 
-- 실질적이니 데이터를 물리적으로 저장하지 않고, 퀴러만 저장했지만
...
select * from hansik;		-- 실제 스키마에는 생기지 않았음. 가상의 테이블이 생기는 것.
...
select * from hansik;
select * from tbl_menu;		-- 원본 값도 바뀐다. 
