...
-- tbl_menu 에서 메뉴 코드, 이름, 가격을 조회할 건데, 가격이 20000 초과인 녀석들만 조회해줘
select
	menu_code,
    menu_name,
    menu_price
...
-- tbl_menu 테이블에서 메뉴코드, 이름, 가격을 조회할 건데 10000원 이상 20000원 이하 조회해줘
select
	menu_code,
    menu_name,
    menu_price
...
from
	tbl_menu
where
	category_code in (4, 5, 6);		-- 위의 식을 in()으로 간단하게 not in() 도 가능
...
from
	tbl_category
where
	ref_category_code is null;		-- null 은 =null 이라고 하면 안 된다. 부정은 is not
